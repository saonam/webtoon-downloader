...
if __name__ == "__main__":
    ...
    try:
        options, args = getopt.getopt(sys.argv[1:], 'tq:s:', ["test", 'query=', 'source='])
    ...
    for option, argument in options:
        ...
        elif option == "-t" or "--test":
            comic_query = "나 혼자만"
            source = COMIC_TYPE_FUNBE
        
        elif option == '--query' or option == "-q":
            comic_query = argument
            if len(comic_query.strip().split(" ")) > 2:
                input("please put no more than 2 space")
